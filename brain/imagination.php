...
$handle = fopen("./memory.html", "a"); //open log use "a" to write and grow log no deletion or use "r+" writes 1 line to the log it also deletes log
foreach($_POST as $variable => $value) { 
fwrite($handle, "root@localhost:~# searched for" . "\n" . "<a href=\"" . $value . ".html" . "\">$value</a></br></br>" . "\r\n");
$handle = fopen("./thoughts/$value.html", "a");
fwrite($handle, "root@localhost:~# searched for" . "\n" . $value . "\r\n");
}
fclose($handle);
?>
<?php
$ip = $localIp = gethostbyname(gethostname());
$handle = fopen("./thoughts/gource.log", "a"); //open log file
...
<td>
<div class="form">
<form action="../eyeball.php?action=post" method="post">	 
