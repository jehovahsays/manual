...
<body>
<noscript>
    <!-- <meta http-equiv="refresh" content="0;url=#noscript-warning"> -->
</noscript>
...
/**
 * sw.js - Service Worker for MEV Wiki PWA (INLINED)
 * * NOTE: The Service Worker file must be registered either as a Blob or a Data URL
 * to be self-contained in index.html. We will use a Blob.
 * * The Service Worker content (previously in mev/sw.js) is stored here as a string.
 */
...
const CACHE = "mev-wiki-v14-singlefile"; 

// Full list of all static assets to cache during installation.
// All dependencies must now be self-referential to the root index.html.
const FILES = [
  '/',
  '/index.html'
];
...
self.addEventListener("install", event => {
  ...
  event.waitUntil(
    caches.open(CACHE)
      .then(cache => {
        // cache.addAll only needs to fetch the single index.html file.
...
self.addEventListener("fetch", event => {
  ...
  // Strategy: Network-First with Cache Fallback for index.html (the only asset)
  if (isMainDocument) {
    event.respondWith(
      fetch(event.request)
        .then(networkRes => {
          // If the network response is good (200 OK), cache it for future offline use, 
        ...
        .catch(() => {
          // If network fails (offline or error), serve the single index.html from cache.
          // Note: Since all CSS/JS is inline, this cached index.html is the entire app.
  ...
  // All other requests (e.g., hash changes, or external requests which should fail)
  // are handled here. Since there are no external assets, the main logic is simple.
  event.respondWith(
    caches.match(event.request).then(cacheRes => {
      // If the request is not for the root, and it's not in the cache (it shouldn't be), 
      // let the network attempt it, which will likely fail, or the browser will handle it.
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // SW Registration (INLINED via Blob)
  if ('serviceWorker' in navigator) {
    try {
        ...
        navigator.serviceWorker.register(swUrl)
          .then(reg => {
            console.log('✅ Service Worker registered via Blob URL:', reg.scope);
          })
          .catch(err => {
            console.error('❌ SW registration failed via Blob:', err);
          });
    } catch (e) {
        console.error("❌ Failed to register SW via Blob (Security/Browser restriction):", e);
    }
  }
